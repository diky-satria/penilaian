...
<div class="container-fluid px-4">
    <div class="content-top">
        <h1 style="font-size:20px;">Wali kelas - Santri</h1>
        <ol class="breadcrumb">
    ...
    <div class="row" style="padding:0 10px;">
        <div class="col-12 col-md-12 bg-table">
            ...
            <div class="row">
                <table class="table" style="font-size:14px;" id="tab-santri">
                    <thead>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="" id="form-edit">
            <input type="hidden" name="id_wali_kelas_santri" id="id_wali_kelas_santri">
                ...
                <div class="col-12 col-md-12">
                        ...
                        <table class="table table-sm" style="font-size:14px;border:1px solid transparent;" id="tab-santri">
                            <thead>
                                ...
                                <tr>
                                    <td>Nama Murid</td>
                                    <td>:</td>
                                    <td><b><span id="td_nama_santri"></span></b></td>
                                    <td>Nomor Induk</td>
                                    <td>:</td>
                    ...
                    <div class="row mb-4">
                        <table class="table table-sm table-bordered" style="font-size:14px;">
                            ...
                            <tr>
                                <td colspan="3"><b><i>Catatan Wali Santri : <span id="td_catatan_wali_kelas"></span></i></b></td>
                            </tr>
                        </table>
                    </div>
                    <div class="row mb-4">
                        <table class="table table-sm table-bordered" style="font-size:14px;border:1px solid transparent;">
                            ...
                            <tr>
                                <td></td>
                                <td></td>
                                <td>Tanggal : <span id="td_tanggal"></span></td>
                            </tr>
                            <tr>
                                <td>Mengetahui,</td>
                                <td></td>
                            ...
                            <tr>
                                <td><b><u id="td_wali_santri"></u></b></td>
...
$(document).ready(function(){
            ...
            $('.tbody-santri').html('')
                $.each(response.data, function(key, value){
                    ...
                    if(value.jumlah_nilai != value.nilai_akhir){
                        ...
                        var nilai = '<button data-bs-toggle="modal" data-bs-target="#staticBackdropNilai" class="btn btn-sm btn-info nilai_raport" wali_kelas_id="'+value.wali_kelas_id+'" wali_kelas_santri="'+value.id+'" santri_client="'+value.santri_id+'" tp="'+value.tahun_pelajaran+'" sm="'+value.semester+'" peringkat="'+baris+'">Nilai</button>'
                        var print = '<a target="blank" href="/wali_kelas/'+value.wali_kelas_id+'/wali_kelas_santri/'+value.id+'/santri_client/'+value.santri_id+'/tp/'+value.tahun_pelajaran+'/sm/'+value.semester+'/peringkat/'+baris+'/print" class="btn btn-sm" style="background-color:brown;color:white;">Print</a>'
                    }

                    $('.tbody-santri').append('<tr>\
                                        <td>'+baris+'</td>\
                                        ...
                                        <td>'+status+'</td>\
                                        <td>\
                                            <button class="btn btn-sm btn-success btn-edit" data-bs-toggle="modal" data-bs-target="#staticBackdropEdit" wali_kelas_santri_id="'+value.id+'" wali_kelas_id="'+value.wali_kelas_id+'" santri_id="'+value.santri_id+'" tahun_pelajaran="'+value.tahun_pelajaran+'" semester="'+value.semester+'">Edit</button>\
                                            '+nilai+'\
                                            '+print+'\
                                        </td>\
                                    </tr>')
                })
                $('#tab-santri').DataTable({
                    language: {
    ...
    $(document).on('click', '.btn-edit', function(){
        ...
        var wali_kelas_santri_id = $(this).attr('wali_kelas_santri_id')
        var wali_kelas_id = $(this).attr('wali_kelas_id')
        var santri_id = $(this).attr('santri_id')
        ...
        $.ajax({
            ...
            +'/wali_kelas_santri/'+wali_kelas_santri_id+'/santri_client/'+santri_id+'/tp/'+tahun_pelajaran+'/sm/'+semester+'/data_edit',
            success: function(response){
                $('#id_wali_kelas_santri').val(response.data.id)
    ...
    $(document).on('submit', '#form-edit', function(e){
        ...
        // ambil id
        let id = $('#id_wali_kelas_santri').val()
        ...
        $.ajax({
            type: "POST",
            url: "/wali_kelas_santri_admin/"+id,
            data: formData,
            contentType: false,
            processData: false,
            success: function(){
                $('#tab-santri').DataTable().destroy()
                ambilData()
    ...
    $(document).on('click', '.nilai_raport', function(){
        ...
        var wali_kelas_id = $(this).attr('wali_kelas_id')
        var wali_kelas_santri = $(this).attr('wali_kelas_santri')
        var santri_client = $(this).attr('santri_client')
        var tp = $(this).attr('tp')
        var sm = $(this).attr('sm')
        ...
        $.ajax({
            type: 'GET',
            url: '/wali_kelas/'+wali_kelas_id
            +'/wali_kelas_santri/'+wali_kelas_santri+'/santri_client/'+santri_client+'/tp/'+tp+'/sm/'+sm+'/peringkat/'+peringkat,
            success: function(response){
                
                $('#td_kelas').append(response.santri.kelas)
                $('#td_semester').append(response.tahun_pelajaran.semester)
                $('#td_tahun_pelajaran').append(response.tahun_pelajaran.tahun_pelajaran)
                $('#td_nama_santri').append(response.santri.nama_santri)
                $('#td_nisn').append(response.santri.nisn)
                ...
                $('#td_jumlah_santri').append(response.jumlah_santri)

                if(response.wali_kelas_santri.kelakuan != null){
                    $('#td_kelakuan').append(response.wali_kelas_santri.kelakuan)
                }else{
                    $('#td_kelakuan').append('-')
                }
                if(response.wali_kelas_santri.kerajinan != null){
                    $('#td_kerajinan').append(response.wali_kelas_santri.kerajinan)
                }else{
                    $('#td_kerajinan').append('-')
                }
                if(response.wali_kelas_santri.kebersihan != null){
                    $('#td_kebersihan').append(response.wali_kelas_santri.kebersihan)
                }else{
                    $('#td_kebersihan').append('-')
                }
                $('#td_sakit').append(response.wali_kelas_santri.sakit)
                $('#td_izin').append(response.wali_kelas_santri.izin)
                $('#td_alpha').append(response.wali_kelas_santri.alpha)
                $('#td_catatan_wali_kelas').append(response.wali_kelas_santri.catatan_wali_kelas)

                $('#td_tanggal').append(response.tanggal)

                $('#td_wali_santri').append(response.santri.wali)
