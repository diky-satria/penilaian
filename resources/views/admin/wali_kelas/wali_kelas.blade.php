        ...
        <form action="" id="form-tambah-wali-kelas">
            <div class="form-group mb-3">
                ...
                <select name="ustad" id="ustad" class="form-control fc-edited">
                    <option value="">----</option>
                    @foreach($ustad as $u)
                    <option value="{{ $u->id }}">{{ $u->nama_ustad }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group mb-3">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="" id="form-edit-wali-kelas">
            ...
            <input type="hidden" name="id_ustad" id="id_ustad">

            <div class="form-group mb-3">
                <label>Guru</label>
                <select name="ustad_edit" id="ustad_edit" class="form-control fc-edited">
                    @foreach($ustad as $u)
                    <option value="{{ $u->id }}">{{ $u->nama_ustad }}</option>
                    @endforeach
                </select>
...
<script>
    ...
    $(document).on('click', '.edit-wali-kelas', function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $('#id_ustad').val(response.data.ustad_id)
                $('#ustad_edit').val(response.data.ustad_id)
            },
    ...
    $(document).on('click', '.aktifkan', function(e){
        e.preventDefault()
        let id = $(this).attr('id')
        let id_ustad = $(this).attr('id_ustad')
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/wali_kelas/aktifkan/"+id+"/"+id_ustad,
                    success: function(response){
                        if(response.duplikat){
