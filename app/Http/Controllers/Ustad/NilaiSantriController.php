...
{
    public function index($tahun_pelajaran_id, $mapel_ustad_id)
    {
        $mapel_ustad = MapelUstad::find($mapel_ustad_id);

        if(!$mapel_ustad || $mapel_ustad->ustad->user->id !== Auth::user()->id){
            return redirect('/penilaian');
        }

        $nilai = Nilai::where('tahun_pelajaran_id', $tahun_pelajaran_id)
                        ->where('mapel_ustad_id', $mapel_ustad_id)
                        ->get();
        ...
        if(request()->ajax()){  
            return datatables()->of($nilai)
                                ...
                                ->addColumn('action', function($nilai){
                                    $button = '<button class="btn btn-sm btn-success ms-1 edit-nilai-santri" data-bs-toggle="modal" data-bs-target="#staticBackdropEdit" id_nilai="'.$nilai->id.' id_thn_pelajaran="'.$nilai->tahun_pelajaran_id.'" id_mapel_ustad="'.$nilai->mapel_ustad_id.'"">Edit</button>';
                                    return $button;
                                })
        ...
        return view('ustad.nilai_santri.nilai_santri', [
            'mapel_ustad' => $mapel_ustad
        ]);
    }

    public function detail($id_thn_pelajaran, $id_mapel_ustad, $id_nilai)
    {
